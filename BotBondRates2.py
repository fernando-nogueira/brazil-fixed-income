...
df_base = pd.read_excel(f'{path}/BondRates2.xlsx')

if df_base['Data'].iloc[len(df_base) - 1] == ytday.strftime("%d/%m/%Y"):
    pass
else: 
    df_ltn = pd.read_excel(save, sheet_name = 'LTN', header = 3)
    df_ltn = df_ltn[['Data de Vencimento', 'Tx. Compra', 'Tx. Venda', 'Tx. Indicativas', 'PU']].dropna()
    df_ltn['Título'] = 'LTN'

    df_lft = pd.read_excel(save, sheet_name = 'LFT', header = 3)
    df_lft = df_lft[['Data de Vencimento', 'Tx. Compra', 'Tx. Venda', 'Tx. Indicativas', 'PU']].dropna()
    df_lft['Título'] = 'LFT'

    df_ntnb = pd.read_excel(save, sheet_name = 'NTN-B', header = 3)
    df_ntnb = df_ntnb[['Data de Vencimento', 'Tx. Compra', 'Tx. Venda', 'Tx. Indicativas', 'PU']].dropna()
    df_ntnb['Título'] = 'NTN-B'

    df_ntnf = pd.read_excel(save, sheet_name = 'NTN-F', header = 3)
    df_ntnf = df_ntnf[['Data de Vencimento', 'Tx. Compra', 'Tx. Venda', 'Tx. Indicativas','PU']].dropna()
    ...
    df.to_excel('BondRates/BondRates2.xlsx')
